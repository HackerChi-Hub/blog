...
async function localizeNotionImage(url, keyHint = 'image') {
  ...
  let baseName = 'image';
  try {
    baseName = decodeURIComponent(new URL(url).pathname.split('/').filter(Boolean).pop() || 'image');
  } catch {}
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (is429 && attempt < maxRetries) {
        ...
        console.warn(`[${label}] 429 rate limited, retry ${attempt + 1}/${maxRetries} in ${delay}ms`);
        await sleep(delay);
      } else {
        throw err;
      }
