...
const officialBlocksCache = new Map();
let officialRequestTail = Promise.resolve();
...
// 官方 Notion Integration API 的限速队列。静态构建会并行请求多个页面，
// 不串行时很容易触发 429；这里把正文请求压到约 2.5 次/秒，并对 429/5xx 重试。
async function officialRequest(fn, label, maxRetries = 4) {
  const run = officialRequestTail.then(async () => {
    const elapsed = Date.now() - lastOfficialRequestAt;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
      try {
        ...
        const delay = Math.max(retryAfter, 800 * Math.pow(2, attempt));
  ...
  officialRequestTail = run.catch(() => null);
  return run;
}

async function listOfficialChildren(blockId) {
  const results = [];
  let cursor;
  do {
    const response = await officialRequest(
...
async function hydrateOfficialChildren(blocks, depth = 0) {
  if (depth >= 12) return blocks;
  for (const block of blocks) {
    if (!block?.has_children) continue;
    block.children = await listOfficialChildren(block.id);
    await hydrateOfficialChildren(block.children, depth + 1);
  }
  return blocks;
}

async function getOfficialPageBlocks(pageId) {
  if (!officialBlocksCache.has(pageId)) {
    officialBlocksCache.set(
      pageId,
      listOfficialChildren(pageId).then((blocks) => hydrateOfficialChildren(blocks))
    );
  }
  try {
    return await officialBlocksCache.get(pageId);
  } catch (error) {
    officialBlocksCache.delete(pageId);
    throw error;
  }
}

const officialFileUrl = (block) => {
  const payload = block?.[block?.type];
  if (!payload || !['image', 'file', 'pdf', 'video', 'audio'].includes(block.type)) return '';
  const file = payload[payload.type];
  return file?.url || '';
};

async function localizeOfficialBlockAssets(blocks, slug) {
  for (const block of blocks || []) {
    const remoteUrl = officialFileUrl(block);
    if (remoteUrl) {
      const payload = block[block.type];
      const localized = await localizeNotionImage(remoteUrl, `${slug}-${block.id}`);
      if (payload?.type && payload[payload.type]) payload[payload.type].url = localized;
    }
    if (block.children?.length) {
      await localizeOfficialBlockAssets(block.children, slug);
    }
  }
  return blocks;
}

function firstOfficialImage(blocks) {
  for (const block of blocks || []) {
    if (block?.type === 'image') {
      const url = officialFileUrl(block);
      if (url) return { url, blockId: block.id };
    }
    const nested = firstOfficialImage(block?.children || []);
...
async function localizeNotionImage(url, keyHint = 'image') {
  ...
  const ext = (
    baseName.match(/\.(jpg|jpeg|png|gif|webp|svg|avif|pdf|zip|txt|md|csv|docx?|xlsx?|pptx?|mp3|m4a|wav|mp4|mov|webm)$/i) ||
    ['.bin']
  )[0];
...
// 复用 public/downloads 中已经落盘的稳定素材。
// Notion 偶发 403 时，公告二维码等旧素材仍应继续显示。
function findLocalizedAssetByPrefix(prefix) {
  if (typeof window !== 'undefined' || !prefix) return null;
  try {
    const fs = require('fs');
    const path = require('path');
    const downloadDir = path.join(process.cwd(), 'public', 'downloads');
    ...
    const fileName = fs
      ...
      .find((name) => name.startsWith(prefix) && fs.statSync(path.join(downloadDir, name)).size > 0);
...
// 首页构建时优先复用当前线上文章已经验证过的 og:image。
// 这样 Notion 非公开页面端点偶发 403 时，不会把大量文章退化成同一张默认横幅。
async function getPublishedCoverFromSite(slug) {
  if (!slug || process.env.DISABLE_LIVE_COVER_FALLBACK === '1') return null;
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://hyphentech.top';

  try {
    const { execFileSync } = require('child_process');
    const articleUrl = `${siteUrl.replace(/\/$/, '')}/${encodeURIComponent(slug)}/`;
    ...
    const coverUrl = match[1]
      ...
      .replace(/&quot;/g, '"');
    try {
      const parsed = new URL(coverUrl, siteUrl);
      // 保留完整线上地址：有些历史封面只存在于已发布站点，并未回写到源码 public/。
      // 构建预览和新首页都可直接复用这份稳定静态资产。
      return parsed.toString();
    } catch {
      return coverUrl;
    }
  } catch (error) {
    console.warn(`[getPublishedCoverFromSite] ${slug}:`, error?.message || error);
    return null;
  }
}
...
async function withRetry(fn, label = 'notion', maxRetries = 3, baseDelay = 2000) {
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // 403 通常是 Notion 页面端点的持续拒绝，快速重试一次后转线上快照；
      // 429 才使用完整的指数退避。
      const retryLimit = statusCode === 403 || err?.message?.includes('403') ? 1 : maxRetries;
      if (isTransient && attempt < retryLimit) {
        const delay = baseDelay * Math.pow(2, attempt);
        console.warn(
          `[${label}] HTTP ${statusCode || '403/429'}, retry ${attempt + 1}/${retryLimit} in ${delay}ms`
        );
        await sleep(delay);
      } else {
        throw err;
...
export async function getPostCovers(posts) {
  const covers = await Promise.all(
    posts.map(async (post) => {
      ...
      // 已发布文章优先复用线上经过验证的 og:image，避开 Notion 正文端点偶发 403。
      const publishedCover = await getPublishedCoverFromSite(key);
      if (publishedCover) {
        return { id: post.id, cover: publishedCover };
      }
      // 新文章还没有线上快照时，再从 Notion 正文中提取第一张图片。
      try {
        const blocks = await localizeOfficialBlockAssets(
          await getOfficialPageBlocks(post.id),
          key
        );
        const imageData = firstOfficialImage(blocks);
        const cover = imageData?.url || null;
        return {
          id: post.id,
          cover: cover || findLocalizedAssetByPrefix(`cover-${key}-`) || null,
        };
      } catch (err) {
        console.warn(`[getPostCovers] Failed for ${post.id}:`, err.message);
        return {
          id: post.id,
          cover: findLocalizedAssetByPrefix(`cover-${key}-`) || null,
        };
      }
    })
  );
  ...
  try {
    ...
    const notices = await Promise.all(
      response.results.map(async (page) => {
        ...
        const localizedImage = await localizeNotionImage(imageUrl, `notice-${page.id}`);
        return {
          id: page.id,
          title:
          ...
          image: localizedImage || findLocalizedAssetByPrefix(`notice-${page.id}-`),
...
export async function getPostBySlug(slug) {
  ...
  try {
    // 正文主通道必须使用带 Integration token 的官方 API。
    // 旧实现的 notionApi.getPage 走非官方公开页面端点，私有页面会持续 403。
    const officialBlocks = await localizeOfficialBlockAssets(
      await getOfficialPageBlocks(post.id),
      post.slug || post.id
    );

    // 提取封面图：优先使用页面封面，否则从内容中提取第一张图片
    let imageUrl = await localizeNotionImage(
      post.pageCover || null,
      `cover-${post.slug || post.id}`
    );
    let coverBlockId = null;

    const imageData = firstOfficialImage(officialBlocks);
    ...
    console.error(
      '[getPostBySlug] official Notion blocks API failed:',
      error?.body || error
    );
    throw error;
  }
