...
export async function getPosts() {
  ...
  try {
    ...
    const posts = response.results.map((page) => {
      ...
      const categoryMultiSelect = properties[PROP.category]?.multi_select || [];
      const tagMultiSelect = properties[PROP.tags]?.multi_select || [];
      ...
      return {
        ...
        date,
        summary,
        categories: formatMultiSelect(categoryMultiSelect),
        categoryNames: formatMultiSelectNames(categoryMultiSelect),
        tags: formatMultiSelect(tagMultiSelect),
      };
    });
...
export async function getNotices(limit = 4) {
  ...
  try {
    ...
    const notices = response.results.map((page) => {
    ...
    console.log(
      '[getNotices] notices:',
      notices.map((n) => ({ id: n.id, title: n.title }))
    );
