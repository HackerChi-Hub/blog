...
const CONTENT_MODE_VALUES = new Set(['dual', 'notion-only', 'markdown-only']);
...
export function getContentMode() {
  const requested = String(process.env.BLOG_CONTENT_MODE || 'markdown-only').trim().toLowerCase();
  if (!CONTENT_MODE_VALUES.has(requested)) {
    throw new Error(
      `BLOG_CONTENT_MODE 必须是 dual、notion-only 或 markdown-only，当前为：${requested}`
    );
  }
  return requested;
}
...
function parseMarkdownFile(filePath, { renderBody = false } = {}) {
  ...
  const notionId = asString(parsed.data.notion_id);
  ...
  const id = asString(parsed.data.id || notionId || `markdown:${slug || path.basename(filePath)}`);
  const markdownSource = preprocessObsidianMarkdown(parsed.content.trim());

  const post = {
    id,
    rawId: notionId ? notionId.replace(/-/g, '') : id,
    notionId,
    slug,
    legacyPaths,
    title,
