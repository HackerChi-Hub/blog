...
MiniMax 为什么要开源这个模型，又图什么？看它的切法就明白了。放出来的是 33B 的生成模型本体，SGLang、vLLM、diffusers、ComfyUI 都能直接部署；最影响成片质量的两段，读懂大白话的 Context-IR 和 2K 重生成，留在自家开放平台的接口上。官方给的 2K 复现流程，同样要调它的接口。我的理解是：开放权重拿生态，完整效果留在平台，两头都不耽误。本地这条路，代价落在你自己身上：机器、电费和等待时间，一条 3 秒视频 7.5～10 分钟；换来的是图片不上传，也不经过托管链路里的自动审核。
...
![四组对照：左边是交给模型的参考图，角标就是 Picture 编号；右边是每条视频第 0.8、1.8、2.8 秒的三帧](/obsidian-assets/localbrain-h3-multi-reference/image-summary-fb212461b6.jpg)
