---
title: 只开localhost，本地AI就安全吗？
slug: localhost-local-ai-security
status: published
date: 2026-09-09
updated: 2026-09-09
summary: 只监听 127.0.0.1，不代表浏览器里的网页碰不到它。三个近期官方安全公告，加一组完全隔离的 CORS 实测，拆开本地 AI 最容易被误解的安全边界。
categories:
  - 学习思考
tags:
  - AI
  - 热点速递
cover: /obsidian-assets/localhost-local-ai-security/cover-affefce7bf.jpg
brand_slogan: 让AI成为你的超能力
legacy_paths: []
---

> [!abstract]
> 网页、跨域、本地接口：127.0.0.1 挡住了外部机器，却不一定挡得住浏览器。
> 黑粉科技 · 2026-09-09

![本地服务、浏览器与恶意网页之间，真正的边界不只有监听地址](/obsidian-assets/localhost-local-ai-security/image-cover-wide-affefce7bf.jpg)
...
![三份官方安全公告：漏洞类型、受影响版本与修复边界](/obsidian-assets/localhost-local-ai-security/image-advisory-matrix-2ef62a5fb8.png)
...
![隔离实测：允许跨域时网页读到 200 JSON；去掉许可后，浏览器阻止页面读取响应](/obsidian-assets/localhost-local-ai-security/image-cors-test-90b8cc916c.png)
...
- 检查浏览器边界：不要给 CORS 填 `*`；只允许真正需要的 Origin，并对写操作增加 CSRF 防护。
- 缩小本地 API 权限：生成接口不该天然拥有任意 shell、任意文件与系统钥匙串能力。
...
> [!summary] 一句话记住这篇
> 外部机器通常不能直连 localhost，但浏览器中的网页可以从本机发起请求；CORS 决定网页能否读取跨域响应，却不能替代认证、CSRF 防护和最小权限。官方公告要按原始严重度和版本边界写；本次实测完全隔离，没有访问任何真实 AI 服务，临时端口已关闭。
