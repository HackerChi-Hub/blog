---
title: 8 GB 显存也能跑本地 AI：Windows 与 Mac 怎么选？
slug: localbrain-windows-nvidia-first-release
status: published
date: 2026-09-04
updated: 2026-09-04
summary: 不讨论“软件有没有 Windows 版”这种表面问题，而是回答两件实际的事：8 GB NVIDIA 显卡能把哪些本地模型和工具跑起来，Mac 的统一内存与完整多媒体链又强在哪里。
categories:
  - 技术分享
tags:
  - AI
  ...
  - 自制软件
  - LocalBrain
cover: /obsidian-assets/localbrain-windows-nvidia-first-release/cover-2.35x1.jpg
legacy_paths: []
---

> [!abstract]
> Windows 负责把 NVIDIA 显卡变成本地文本代理，Mac 负责用统一内存承接更大模型和完整多媒体链。两边不是谁取代谁，而是两套不同的本地 AI 解法。
> 2026-09-04·黑粉科技

# 8 GB 显存也能跑本地 AI：Windows 与 Mac 怎么选？

![8 GB NVIDIA 显卡与大内存 Mac 是两条不同的本地 AI 路线：左侧标题、本人视线和两类设备共同指向本地计算核心](/obsidian-assets/localbrain-windows-nvidia-first-release/cover-2.35x1.jpg)
...
![GitHub 官方发布页已经列出 Windows 10/11、NVIDIA 驱动门槛、8 GB 显存适用范围和当前限制；这张图是发布证据，不是界面示意图](/obsidian-assets/localbrain-windows-nvidia-first-release/official-windows-release.webp)
...
![同一台 8 GB Windows 笔记本的两款模型实测：73.9 与约 60.8 token/s；图表由本文实测记录整理，不是跨设备排行榜](/obsidian-assets/localbrain-windows-nvidia-first-release/windows-8gb-benchmark.webp)
...
![Mac 端把统一内存分成系统与 AI 模型两笔显示；当前有一颗约占 51 GB 的 GGUF 模型在本机运行](/obsidian-assets/localbrain-windows-nvidia-first-release/mac-local-model-home.webp)
...
![Mac 的设置页把本地系统工具、文字与语音、GGUF、视频、图像、音乐等运行环境集中管理；这部分目前不是 Windows 首版的能力](/obsidian-assets/localbrain-windows-nvidia-first-release/mac-local-tools-settings.webp)
...
![模型发现页会先测试 ModelScope、HF-Mirror 与 Hugging Face，再按当前网络选择下载源；卡片能力标签用于说明已验证能力，不把模型名直接等同于能力](/obsidian-assets/localbrain-windows-nvidia-first-release/mac-model-discovery.webp)
...
> [!tip]
> 下载页：https://github.com/HackerChi-Hub/localbrain-releases/releases（Windows 版在 v1.2.50 预发布下）

> [!summary] 8 GB 显存的 Windows 笔记本，本地文本模型加本地工具这条线已经能用了
> 两款 8 GB 档模型分别跑到 73.9 和约 60.8 token/s，窗口 32K 到 55K，联网、文件、文档三类工具在修完那三处之后都通了。多媒体那一半短期内不会来 Windows，Linux 没有时间表，自动更新要等私钥挪过来。我的判断只覆盖 8 GB 这一档——更大显卡上的窗口和卸载路径是同一套推导，但没有实机数据，用 12、16、24 GB 显卡的人试完请把结果发给我。
