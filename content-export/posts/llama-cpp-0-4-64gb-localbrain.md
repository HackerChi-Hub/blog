...
tags:
  - AI
  - 热点速递
cover: /obsidian-assets/llama-cpp-0-4-64gb-localbrain/cover-b591828048.jpg
brand_slogan: 让AI成为你的超能力
legacy_paths: []
---

> [!abstract]
> 它没把 64GB 变成 128GB，但把硬墙从“文件总大小”推到了“活跃工作集、SSD 延迟与长上下文”。
> 黑粉科技 · 2026-09-10
...
![llama.cpp 0.4 把懒加载、流式量化、新架构与 Apple RDMA 放进同一个里程碑](/obsidian-assets/llama-cpp-0-4-64gb-localbrain/image-release-map-6beba31c12.png)
...
| --- | --- | --- |
| `--lazy-mode` | 巨型低频张量被迫全程常驻 | 总文件可以超过内存，但要付 SSD 延迟 |
| 量化器 `--max-buffer-size` | 巨型 embedding 在量化阶段撑爆 RAM | 默认用 8GB 缓冲做 row-slab 流式处理 |
...
![LocalBrain 现有实测：84.93GB 模型通过懒加载进入 64GB Mac，但上下文仍有明确上限](/obsidian-assets/llama-cpp-0-4-64gb-localbrain/image-working-set-fe0f0b6b47.png)
...
![上游数据显示 RDMA 明显减少两机 TCP 损耗，但两机解码仍没有超过对照的单机成绩](/obsidian-assets/llama-cpp-0-4-64gb-localbrain/image-rdma-benchmark-e2fd0825dd.png)
...
![LocalBrain 目前处于验证收口阶段：公开版、本地版与未发布修复严格分层](/obsidian-assets/llama-cpp-0-4-64gb-localbrain/image-localbrain-progress-fbe1f1b44c.png)
...
- **32–64GB 用户：** 不要因为有 `--lazy-mode` 就盲下任何大于内存的模型。先确认它是否真有可按需保留的巨型低频张量，并留出系统和 KV 缓存余量。
- **64GB 跑 Flash-Next：** 把 174K 附近视为这台机器与当前量化的已测区间，不把 262K 启动成功当成可用证明。
