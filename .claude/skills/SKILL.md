...
1. **Notion页面封面** (最高优先级) 🎯
   - 在Notion页面手动设置的封面
   - 代码检查: `page.cover.external.url` 或 `page.cover.file.url`
   
2. **文章内容第一张图片** (默认) 📸
   - 如果未设置页面封面，自动使用文章第一张图片
   - 代码检查: `extractFirstImageFromRecordMap(recordMap)`
...
### 实际应用建议
- **推荐**: 设置Notion页面封面，确保封面显示正确
- **默认**: 如果不设置页面封面，确保文章第一张图片适合做封面
- **避免**: 文章第一张图片不适合作为封面时，务必设置页面封面
